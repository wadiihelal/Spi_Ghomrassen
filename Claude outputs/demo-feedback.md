...
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(mvn *), Bash(npm *), Bash(npx *), Bash(git *), Bash(java *), Bash(curl *), Bash(find *), Bash(ls *), Bash(unzip *), Bash(docker *), Bash(python3 *), Bash(pip *), Bash(pip3 *), Bash(playwright *), Bash(pdftotext *), Bash(pdftoppm *), Bash(pdfinfo *), Bash(pdffonts *)
---
...
- **la reconstruction de la source du manuel** au Lot 2 : demande d'abord a Wadii s'il a encore
  le script qui a produit le PDF ;
...
- **Le manuel (Lot 2) se genere, il ne s'edite pas a la main.** Le PDF dans `docs/` est un
  artefact de `docs/manuel/build.py`. Les captures d'ecran sont produites par `captures.py`
  **uniquement contre le jeu `demo`** (pile locale `docker-compose.demo.yml` ou serveur de
  test) — jamais contre des donnees reelles. Elles trouvent les elements par leur libelle
  francais exact : n'ajoute pas de `data-testid` au frontend pour contourner un libelle.
- **Aucun paragraphe du manuel 1.0 ne disparait** : `check_parity.py` doit passer avant de
  remplacer le PDF.
...
Lot 2 en plus : `python3 docs/manuel/build.py` depuis un clone propre produit le PDF ;
`python3 docs/manuel/captures.py --base-url http://localhost` contre la pile demo produit
les 34 captures ; `check_parity.py` passe.
