...
public interface ExpenseRepository extends JpaRepository<Expense, Long>, JpaSpecificationExecutor<Expense> {
     ...
     */
    @Query(value = """
            select new com.promoteur.app.dto.report.AmountByLabelDto(e.category.name, sum(e.amountTtc))
            from Expense e
            where (:projectId is null or e.project.id = :projectId)
              and (:from is null or e.expenseDate >= :from)
              and (:to is null or e.expenseDate <= :to)
    ...
    @Query(value = """
            select new com.promoteur.app.dto.report.AmountByLabelDto(e.project.name, sum(e.amountTtc))
            from Expense e
            where (:projectId is null or e.project.id = :projectId)
              and (:from is null or e.expenseDate >= :from)
              and (:to is null or e.expenseDate <= :to)
    ...
    @Query("""
            select new com.promoteur.app.dto.report.CountAndTotal(count(e), coalesce(sum(e.amountTtc), 0))
            from Expense e
            where (:projectId is null or e.project.id = :projectId)
              and (:from is null or e.expenseDate >= :from)
              and (:to is null or e.expenseDate <= :to)
