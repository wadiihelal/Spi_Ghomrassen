...
ALTER TABLE apartments ADD COLUMN sales_status VARCHAR(30);
ALTER TABLE apartments ADD COLUMN block VARCHAR(50);
ALTER TABLE apartments ADD COLUMN floor_number INTEGER;

UPDATE apartments SET sales_status = 'AVAILABLE' WHERE sales_status IS NULL;

-- Un appartement sous contrat est vendu.
UPDATE apartments SET sales_status = 'SOLD'
 WHERE id IN (SELECT apartment_id FROM client_purchases);

-- Un acquéreur rattaché sans contrat signé vaut une réservation.
UPDATE apartments SET sales_status = 'RESERVED'
 WHERE client_id IS NOT NULL AND sales_status = 'AVAILABLE';

ALTER TABLE apartments ALTER COLUMN sales_status SET NOT NULL;
